...
# reg = register(
#     id='walker-v7',
#     entry_point='bipedal_env_v7:walkerEnv7')



class walkerEnv7(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self, exclude_current_positions_from_observation=True):
        self._exclude_current_positions_from_observation = (exclude_current_positions_from_observation)
        self.model_path = model_path
        # frame_skip = 10
        super(walkerEnv7).__init__()
        ...
        # self.action_space = spaces.Box(high=300, low=-300, shape=(8,))
        self.action_throttle = 1000
        # self.action_space = spaces.MultiDiscrete([2000,2000, 2000, 2000, 2000, 2000])
        # self.action_space = spaces.Discrete(8)
        self.observation_space= spaces.Box(high=np.inf, low= -np.inf, shape=(218,))
    ...
    def mass_center(self, model, sim):
        ...
        xpos = sim.data.xipos
        return (np.sum(mass * xpos, 0) / np.sum(mass))[0].copy()
    ...
    def _get_obs(self):
        ...
        return np.concatenate((
                                ...
                                np.array([int(self.has_ground_contact()[0]), int(self.has_ground_contact()[1])]),
    ...
    def step(self, action):
        ...
        # If a MuJoCo Exception is raised, catch it and reset the environment
            ...
            obs = self.reset()
            return obs, 0, True, {}
        pos_after = self.mass_center(self.model, self.sim)


        alive_bonus = 5.0
        data = self.sim.data
        lin_vel_cost = 1.25 * (pos_after - pos_before) / self.deltaTime
        quad_ctrl_cost = 0.1 * np.square(data.ctrl).sum()
        quad_impact_cost = 0.5e-6 * np.square(data.cfrc_ext).sum()
        quad_impact_cost = min(quad_impact_cost, 10)
        reward = lin_vel_cost - quad_impact_cost + alive_bonus
        qpos = self.sim.data.qpos
        done = bool((qpos[2] < self.healthy_z_range[0]) or (qpos[2] > self.healthy_z_range[1]))
        # reward = self.get_reward()
        # obs = self._get_obs()
        # print(obs)
        if done:
            reward -= 1000
        print(len(np.array([self._get_obs()])))
        # print(self.has_ground_contact())
        # return obs, reward, done_good, {}
        # print(lin_vel_cost ,quad_impact_cost ,alive_bonus)
        return (
